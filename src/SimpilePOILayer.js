L.LatLngBounds.prototype.toSimplePOIBBoxString = function (){
  var a = this._southWest,
  b = this._northEast;
  return [a.lat, a.lng, b.lat, b.lng].join(",");
}

L.SimplePOILayer = L.FeatureGroup.extend({
  options: {
    debug: false,
    minzoom: 15,
    ...
    beforeRequest: function() {
      if (this.options.debug) {
        console.debug('about to query the SimplePOIAPI');
      }
    },
    ...
    if (this._map.getZoom() >= this.options.minzoom) {
      ...
      var bboxList = this._view2BBoxes(
        ...
        for (var i = 0; i < bboxList.length; i++) {
          ...
          var queryWithMapCoordinates = this.options.query.replace(/(BBOX)/g, bbox.toSimplePOIBBoxString());
...
//L.SimplePOILayer = function (options) {
//  return new L.SimplePOILayer(options);
//};
